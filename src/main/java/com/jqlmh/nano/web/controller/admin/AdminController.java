...
public class AdminController extends BaseController {
	...
	public String index(Model model, HttpSession session) {
		//查询文章条数
		Integer postCount = postService.findAllPosts(NanoConst.POST_TYPE_POST).size();
		...
		model.addAttribute("mediaCount", NanoConst.ATTACHMENTS.size());
		return "admin/admin_index";
	}
	...
	public String getLogin(@ModelAttribute("loginName") String loginName,
		...
		String status = "false";
		try {
			User aUser = userService.findAllUser();
			User user = null;
			if (StringUtils.equals(aUser.getLoginEnable(), "false")) {
				...
				if (matcher.find()) {
					user = userService.userLoginByEmail(loginName, NanoUtil.getMD5(loginPwd)).get(0);
				} else {
					user = userService.userLoginByName(loginName, NanoUtil.getMD5(loginPwd)).get(0);
				}
				if (aUser == user) {
					//输入的用户名和密码和数据库的一致,把用户信息保存在session中
					session.setAttribute(NanoConst.USER_SESSION_KEY, user);
					//重置用户的登录状态为正常
					userService.updateUserNormal();
					//userService.updateUserLoginLast(new Date());   //5.10 重复了,没有重复,因为后面会需要这个方法,只是这个重复调用了
					logsService.saveByLogs(new Logs(LogsRecord.LOGIN, LogsRecord.LOGIN_SUCCESS, NanoUtil.getIpAddr(request), NanoUtil.getDate()));
					status = "true";
				} else {
					log.info(loginName + "登录失败!,输入密码为" + loginPwd);
				}
			}
		} catch (Exception e) {
			...
			logsService.saveByLogs(new Logs(LogsRecord.LOGIN, LogsRecord.LOGIN_ERROR + "[" + loginName + "," + loginPwd + "]", NanoUtil.getIpAddr(request), new Date()));
	...
	public String logOut(HttpSession session) {
		User user = (User) session.getAttribute(NanoConst.USER_SESSION_KEY);
		logsService.saveByLogs(new Logs(LogsRecord.LOGOUT, user.getUserName(), NanoUtil.getIpAddr(request), NanoUtil.getDate()));
		session.invalidate();
